...
const char * const BITCOIN_CONF_FILENAME = "dreamcoin.conf";
const char * const BITCOIN_PID_FILENAME = "dreamcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "dreamcoin" is a composite category enabling all Dreamcoin-related debug output
            if(ptrCategory->count(string("dreamcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DreamcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DreamcoinCore
    // Mac: ~/Library/Application Support/DreamcoinCore
    // Unix: ~/.dreamcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DreamcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DreamcoinCore";
#else
    // Unix
    return pathRet / ".dreamcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty dreamcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override dreamcoin.conf
